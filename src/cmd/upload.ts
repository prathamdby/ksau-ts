...
import {
  ...
  getChunkSize, getConfigData,
  selectRemoteAutomatically,
  verifyFileIntegrity,
} from './utils.ts'
import { ProgressTracker, validStyles, type ProgressStyle } from './progress/progress.ts'

export function registerUploadCommand(program: Command): void {
  program
    .command('upload')
    .description('Upload a file to OneDrive')
    .requiredOption('-f, --file <path>', 'Path to the local file to upload (required)')
    .requiredOption('-r, --remote <folder>', 'Remote folder on OneDrive to upload the file (required)')
    .option('-n, --remote-name <name>', 'Optional: Remote filename (defaults to local filename)', '')
    .option('-s, --chunk-size <bytes>', 'Chunk size for uploads in bytes (0 for automatic selection)', '0')
    .option('--retries <n>', 'Maximum number of retries for uploading chunks', '3')
    .option('--retry-delay <ms>', 'Delay between retries in ms', '5000')
    .option('--skip-hash', 'Skip QuickXorHash verification', false)
    .option('--hash-retries <n>', 'Maximum number of retries for fetching QuickXorHash', '5')
    .option('--hash-retry-delay <ms>', 'Delay between QuickXorHash retries in ms', '10000')
    .option('--progress <style>', `Progress bar style for upload visualization:
\tbasic:   [=====>     ] 45% | 5.2MB/s
\tblocks:  █████░░░░░ 45% | 5.2MB/s
\tmodern:  ○○●●●○○○ 45% | 5.2MB/s
\temoji:   🟦🟦🟦⬜⬜ 45% | 5.2MB/s
\tminimal: 45% | 5.2MB/s | 42MB/100MB | ETA: 2m30s`, 'modern')
    .option('--emoji <emoji>', `Custom emoji for emoji progress style. Examples:
...
\t⭐ (yellow star), 🚀 (rocket), 📦 (package)`, '🟦')
    .action(async (opts) => {
      ...
      if (!remoteConfig) {
        try {
          remoteConfig = await selectRemoteAutomatically(fileSize, progressStyle as ProgressStyle)
        } catch (err) {
          console.log('cannot automatically determine remote to be used:', (err as Error).message)
          process.exit(1)
        }
      }
      ...
      if (chunkSizeNum === 0) {
        ...
        console.log('Selected chunk size: ' + chunkSizeBig + ' bytes (based on file size: ' + fileSize + ' bytes)')
      } else {
        chunkSizeBig = BigInt(chunkSizeNum)
        if (chunkSizeBig > maxChunkSize) {
          console.log('Warning: Reducing chunk size from ' + chunkSizeBig + ' to ' + maxChunkSize + ' bytes for reliability')
          chunkSizeBig = maxChunkSize
        } else if (chunkSizeBig % (320n * KibiByte) !== 0n) {
          console.log('Warning: Chunk size ' + chunkSizeBig + ' is not multiple of 320KiB, upload may not be optimal')
        } else {
      ...
      try {
        client = await AzureClient.fromRcloneConfigData(configData, remoteConfig)
      } catch (err) {
      ...
      let tracker: ProgressTracker | null = new ProgressTracker(fileSize, progressStyle as ProgressStyle)
      ...
      if (fileID !== '') {
        ...
        let urlPath = path.join(remoteFolder, localFileName).replace(/\\/g, '/').replace(/ /g, '%20')
        if (remoteFileName) {
          urlPath = path.join(remoteFolder, remoteFileName).replace(/\\/g, '/').replace(/ /g, '%20')
        }
        const downloadURL = baseURL + '/' + urlPath
        console.log(ColorGreen + 'Download URL:' + ColorReset + ' ' + ColorGreen + downloadURL + ColorReset)
